...
export function UserHomePage() {
  ...
  const { data: newReleasesRaw } = vibeApi.useGetNewReleasesQuery({})

  // Normalise every new release into a player-ready Track (audioUrl + coverUrl fully resolved)
  ...
  return (
    <div className="px-4 md:px-8 py-6 space-y-8">
      ...
      {newReleaseTracks.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-heading text-base font-semibold text-white">New Releases</h2>
            <button className="flex items-center gap-1 text-xs text-vibe-text-muted hover:text-white transition-colors">
              Show all <span className="text-vibe-red">→</span>
            </button>
          </div>
            ...
            <div className="flex gap-4 pb-3" style={{ scrollSnapType: "x mandatory" }}>
              {newReleaseTracks.map((track) => (
                <TrendingCard
                  key={track.id}
                  track={track}
                  onPlay={() => dispatch(playTrack({ track, queue: newReleaseTracks }))}
                />
              ))}
            </div>
            <ScrollBar orientation="horizontal" />
...
// ── Trending / New Release card (shared) ─────────────────
